    ...
    useEffect(()=>{
        ...
        setIsLoading(true);
            const fetchapi=async ()=>{
                ...
                setFilteredPlaces([]);
                setRest(data?.filter(place=>place.name && place.num_reviews>0));
                setIsLoading(false);
            }
            fetchapi();
        ...
        <>
         ...
         <Grid container spacing={3} style={{width:"100%"}} >
             ...
             <Grid item xs={12} md={8}>
                 <Map weatherState={weatherState} setChildClicked ={setChildClicked} places={filteredPlaces.length ? filteredPlaces :rest}  coords={coords} setCoords={setCoords} setBounds={setBounds} />
             </Grid>
         </Grid>   
        </>
    )
